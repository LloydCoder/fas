        ...
        for collector,spec in zip(plan.collectors,plan.specs):
            ...
            while attempts<spec.max_attempts:
                ...
                try:
                    ...
                    status=CollectionStatus.INVALID_INPUT
                    error=str(exc)
                    retry=RetryDisposition.DO_NOT_RETRY
                    break
                except PermissionError as exc:
                    status=CollectionStatus.TOOL_ERROR
                    error=str(exc)
                    retry=RetryDisposition.DO_NOT_RETRY
                    break
                    ...
                    retry=RetryDisposition.RETRY if attempts<spec.max_attempts else RetryDisposition.DO_NOT_RETRY
                if retry==RetryDisposition.DO_NOT_RETRY:
                    break
            ...
            for o in batch.observations:
                observations[o.id]=o
            for a in batch.raw_artifacts:
                raw_artifacts[getattr(a,"artifact_id",str(a))]=a
            for r in batch.tool_runs:
                tool_runs[getattr(r,"run_id",str(r))]=r
            skipped+=batch.skipped
            warnings.extend(batch.warnings)
            if plan.fail_fast and status not in {CollectionStatus.SUCCESS,CollectionStatus.PARTIAL}:
                break
