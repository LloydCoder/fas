        ...
        for path in sorted(context.root.rglob("*")):
            try:
                ...
                if size>context.max_file_bytes:
                    b.complete=False
                b.skipped+=1
                b.warnings.append(f"CI/CD file too large: {rel}")
                continue
                data=path.read_bytes()
                digest=__import__("hashlib").sha256(data).hexdigest()
                aid=_stable_id("artifact",f"{context.snapshot_id}|ci-cd|{rel}|{digest}")
                prov=_provenance("ci_cd_hash",rel)
                ...
                b.complete=False
            b.skipped+=1
            b.warnings.append(f"CI/CD unreadable: {exc}")
        return b.build()
