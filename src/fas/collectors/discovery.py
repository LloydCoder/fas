...
def _stable_id(prefix,material):
    alphabet="0123456789ABCDEFGHJKMNPQRSTVWXYZ"; value=int.from_bytes(hashlib.sha256(material.encode()).digest()[:16],"big"); chars=[]
...
def _artifact_type(path):
    name=path.name
    if name in _MANIFEST_NAMES or name.endswith((".lock",".sum")): return ArtifactType.DEPENDENCY_METADATA
    if path.suffix.lower() in {".yaml",".yml",".toml",".ini",".env"}: return ArtifactType.CONFIGURATION
    if path.suffix.lower() in {".sarif",".json"} and any(x in name.lower() for x in ("report","result","scan")): return ArtifactType.TOOL_OUTPUT
    if path.suffix.lower() in _SOURCE_SUFFIXES: return ArtifactType.SOURCE_FILE
        ...
        for root,dirs,files in os.walk(context.root,followlinks=False):
            ...
            for filename in sorted(files):
                if count>=context.max_files:
                    builder.complete=False; builder.warnings.append("max_files reached"); return builder.build()
                path=Path(root)/filename
                try:
                    if path.is_symlink() or not path.is_file(): builder.skipped+=1; continue
...
class CodeDiscoveryCollector:
    name="code-discovery"
    def collect(self,context):
        discovered=RepositoryDiscoveryCollector().collect(context); observations=[]
        for artifact in discovered.artifacts:
            if artifact.type!=ArtifactType.SOURCE_FILE: continue
...
class DependencyDiscoveryCollector:
    ...
    def collect(self,context):
        discovered=RepositoryDiscoveryCollector().collect(context); observations=[]
        for artifact in discovered.artifacts:
            if artifact.type!=ArtifactType.DEPENDENCY_METADATA: continue
...
def _parse_dependency_manifest(name,data):
    ...
    if lower=="pyproject.toml":
        raw=tomllib.loads(data); project=raw.get("project",{})
    ...
    if lower.endswith(".json"):
        raw=json.loads(data); return {"format":Path(name).name,"dependencies":raw.get("dependencies",{}) if isinstance(raw,dict) else {},"dev_dependencies":raw.get("devDependencies",{}) if isinstance(raw,dict) else {}}
    ...
    if lower=="go.mod":
        ...
        for line in data.splitlines():
            stripped=line.strip()
            if stripped.startswith("require ("): block=True; continue
            if block and stripped==")": block=False; continue
            if block and stripped and not stripped.startswith("//"): deps.append(stripped)
