...
class CollectionPipeline:
    def __init__(self,engine,*,normalizer=None): self.engine=engine
    self.normalizer=normalizer or ObservationNormalizer()
    def ingest(self,batch,context):
        if self.engine.scope.analysis_id!=context.analysis_id or self.engine.scope.snapshot_id!=context.snapshot_id:
            raise ValueError("collection scope does not match graph scope")
