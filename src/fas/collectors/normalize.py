...
def _evidence_id(context,observation)->EvidenceId:
    material=json.dumps(observation.model_dump(mode="json"),sort_keys=True,separators=(",",":"),ensure_ascii=False)
...
class ObservationNormalizer:
    def normalize(self,observation,context):
        if observation.analysis_id!=context.analysis_id or observation.snapshot_id!=context.snapshot_id:
            raise NormalizationError("observation is outside collection scope")
