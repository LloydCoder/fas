...
class AgentConfigurationCollector:
    ...
    def collect(self,context):
        ...
        for path in sorted(context.root.rglob("*")):
            try:
                if not path.is_file() or path.is_symlink() or path.name.lower() not in _AGENT_NAMES:
                    continue
                if path.stat().st_size>context.max_file_bytes:
                    b.complete=False
                b.skipped+=1
                b.warnings.append(f"agent config too large: {path}")
                continue
                data=path.read_text(encoding="utf-8")
                digest=__import__("hashlib").sha256(data.encode()).hexdigest()
                ...
                b.complete=False
            b.skipped+=1
            b.warnings.append(f"agent config unreadable: {exc}")
        return b.build()

def _safe_shape(value):
    if isinstance(value,dict):
        return {str(k):_safe_shape(v) for k,v in value.items() if str(k).lower() not in {"token","password","secret","api_key","authorization","credential"}}
    if isinstance(value,list):
        return [_safe_shape(v) for v in value[:1000]]
