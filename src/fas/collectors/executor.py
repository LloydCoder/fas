...
class ExecutionPolicy:
    ...
    def __post_init__(self):
        if self.timeout_seconds<=0 or self.max_output_bytes<1 or self.max_stderr_bytes<1 or self.max_args<1:
...
class SecureExecutor:
    ...
    def run(self,argv:Sequence[str],*,cwd:Path,env:Mapping[str,str]|None=None,cancel:CancellationToken|None=None)->ExecutionResult:
        ...
        if self.policy.allowed_executables and executable not in self.policy.allowed_executables and args[0] not in self.policy.allowed_executables:
            raise PermissionError("executable is not allowlisted")
        root=Path(cwd).resolve()
        if not root.is_dir():
            raise ValueError("cwd must be a directory")
        if any(x in {".."} for x in Path(args[0]).parts):
        ...
        if env:
            for key,value in env.items():
                if key.upper() in {"LD_PRELOAD","LD_LIBRARY_PATH","PYTHONPATH","PYTHONINSPECT"}:
                    continue
        ...
        try:
            ...
            return ExecutionResult(args,None,b"",b"",timed_out=True)
        if cancel and cancel.cancelled:
            self._terminate(process)
            return ExecutionResult(args,None,b"",b"",cancelled=True)
