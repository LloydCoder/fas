...
class ConfigurationCollector:
    ...
    def collect(self,context):
        base=RepositoryDiscoveryCollector().collect(context); b=_BatchBuilder(list(base.artifacts),[],list(base.warnings),base.skipped,base.complete)
        for artifact in base.artifacts:
            if artifact.type not in {ArtifactType.CONFIGURATION,ArtifactType.DEPENDENCY_METADATA}: continue
