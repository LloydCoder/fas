...
class SemgrepAdapter:
    ...
    def parse(self,payload,context,*,raw_artifact_id=None,source_artifact_id=None):
        ...
        for index,item in enumerate(results):
            ...
            path=item.get("path")
            location=None
            if path and source_artifact_id:
                location=SourceLocation(artifact_id=source_artifact_id,path=str(path),line_start=start.get("line"),line_end=end.get("line") or start.get("line"),column_start=start.get("col"),column_end=end.get("col"))
            rule=str(item.get("check_id") or "unknown")
            message=str(extra.get("message") or rule)
            provenance=Provenance(category=ProvenanceCategory.TOOL_OBSERVATION,level=ProvenanceLevel.T2,collector=self.name,collector_version=str(version) if version else None,method="semgrep_json_parse",source=f"results[{index}]",observed_at=now)
