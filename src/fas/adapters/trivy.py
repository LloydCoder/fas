    ...
    def parse(self,payload,context,*,raw_artifact_id=None,source_artifact_id=None):
        ...
        for result_index,result in enumerate(results):
            if not isinstance(result,dict):
                raise TypeError(f"trivy result {result_index} is not an object")
            target=result.get("Target")
            vulnerabilities=result.get("Vulnerabilities") or []
            if not isinstance(vulnerabilities,list):
                raise TypeError(f"trivy vulnerabilities[{result_index}] is not an array")
            for item_index,vuln in enumerate(vulnerabilities):
                if not isinstance(vuln,dict):
                    raise TypeError(f"trivy vulnerability {result_index}:{item_index} is not an object")
