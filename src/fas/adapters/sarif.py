...
class SarifAdapter:
    ...
    def parse(self,payload,context,*,raw_artifact_id=None):
        ...
        for run_index,run in enumerate(raw.get("runs",[])):
            driver=((run.get("tool") or {}).get("driver") or {}); tool_name=str(driver.get("name") or "unknown"); version=driver.get("semanticVersion") or driver.get("version"); now=datetime.now(timezone.utc)
...
def _sarif_location(location:Any,raw_artifact_id):
    ...
    physical=location.get("physicalLocation") or {}; artifact=physical.get("artifactLocation") or {}; uri=artifact.get("uri"); region=physical.get("region") or {}
    if not uri or not raw_artifact_id: return None
    parsed=urlparse(str(uri)); path=parsed.path or str(uri)
