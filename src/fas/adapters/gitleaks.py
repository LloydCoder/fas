...
class GitleaksAdapter:
    ...
    def parse(self,payload,context,*,raw_artifact_id=None):
        ...
        for index,item in enumerate(raw):
            ...
            if path and raw_artifact_id: location=SourceLocation(artifact_id=raw_artifact_id,path=str(path),line_start=item.get("StartLine") or item.get("EndLine"),line_end=item.get("EndLine") or item.get("StartLine"),column_start=item.get("StartColumn"),column_end=item.get("EndColumn"),commit=item.get("Commit"))
