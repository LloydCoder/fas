...
class GitleaksAdapter:
    ...
    def parse(self,payload,context,*,raw_artifact_id=None,source_artifact_id=None):
        ...
        if not isinstance(raw,list):
            raise TypeError("gitleaks JSON report must be an array")
        observations=[]
        now=datetime.now(timezone.utc)
        for index,item in enumerate(raw):
            if not isinstance(item,dict):
                raise ValueError(f"gitleaks result {index} is not an object")
            rule_id=str(item.get("RuleID") or "unknown")
            path=item.get("File") or item.get("SymlinkFile")
            fingerprint=item.get("Fingerprint")
            provenance=Provenance(category=ProvenanceCategory.TOOL_OBSERVATION,level=ProvenanceLevel.T2,collector=self.name,method="gitleaks_json_parse",source=f"[{index}]",observed_at=now)
            location=None
            if path and raw_artifact_id:
                location=SourceLocation(artifact_id=source_artifact_id,path=str(path),line_start=item.get("StartLine") or item.get("EndLine"),line_end=item.get("EndLine") or item.get("StartLine"),column_start=item.get("StartColumn"),column_end=item.get("EndColumn"),commit=item.get("Commit"))
